...
export function accessMemory(
  ...
  // Hebbian link strengthening on co-access
  try {
...
const ENRICHMENT_SIMILARITY_THRESHOLD = 0.3;
const ENRICHMENT_COOLDOWN_HOURS = 1;
const MAX_ENRICHMENT_SIZE = 2000;
...
export async function searchMemories(
  ...
  const results: SearchResult[] = rows.map(row => {
    ...
    // FTS score normalization (BM25-style)
 ...
 * 2. FTS content similarity (Jaccard similarity >= 0.3)
 */
